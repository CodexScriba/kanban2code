...
import { createSavePayload, useTaskEditor, type TaskEditorSavePayload } from '../hooks/useTaskEditor';
...
function frontmatterPreview(task: Task, content: string): string {
  const lines = [
    '---',
    `stage: ${task.stage}`,
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'grid', placeItems: 'center', zIndex: 10 }}>
      <section style={{ width: 'min(920px, 95vw)', maxHeight: '90vh', overflow: 'auto', background: 'var(--vscode-editor-background)', border: '1px solid var(--vscode-panel-border)', borderRadius: 10, padding: 14, display: 'grid', gap: 10 }}>
        ...
        {tab === 'body' ? (
          ...
          <pre aria-label="Frontmatter preview" style={{ margin: 0, maxHeight: 320, overflow: 'auto' }}>{frontmatterPreview(task, draft.content)}</pre>
        )}
