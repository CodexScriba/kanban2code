...
/**
 * CLI adapter for MiniMax (`minimax` command).
 *
 * MiniMax one-shot mode is flag-based (`-p`) and returns plain text when
 * used with print/quiet style flags.
 */
export class MiniMaxAdapter implements CliAdapter {
  buildCommand(
    ...
    // MiniMax prompt style for runner one-shot calls.
    args.push('-p', prompt);
