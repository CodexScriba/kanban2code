...
| Tasks created by agents have valid frontmatter | `tests/agent-service.test.ts` + command behavior | Agent metadata parsing is tested; task creation stores agent assignment in frontmatter |
...
4. Run `Kanban2Code: Copy Task Context (Full XML)` and confirm:
   ...
   - `<context>` includes agent context + selected contexts
