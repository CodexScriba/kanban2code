...
var isSettingsSection = (value) => typeof value === "string" && SETTINGS_SECTIONS.includes(value);
var isTaskInput = (value, allowProjectSlug) => {
  if (!isObject(value)) {
    return false;
  }
  if ("title" in value && value.title !== void 0 && typeof value.title !== "string") {
    return false;
  }
  if ("body" in value && value.body !== void 0 && typeof value.body !== "string") {
    return false;
  }
  if ("stage" in value && value.stage !== void 0 && !isTaskStage(value.stage)) {
    return false;
  }
  if ("role" in value && value.role !== void 0 && typeof value.role !== "string") {
    return false;
  }
  if ("agent" in value && value.agent !== void 0 && typeof value.agent !== "string") {
    return false;
  }
  if ("provider" in value && value.provider !== void 0 && typeof value.provider !== "string") {
    return false;
  }
  if ("model" in value && value.model !== void 0 && typeof value.model !== "string") {
    return false;
  }
  if ("profile" in value && value.profile !== void 0 && typeof value.profile !== "string") {
    return false;
  }
  if ("priority" in value && value.priority !== void 0 && !isPriority(value.priority)) {
    return false;
  }
  if ("tags" in value && value.tags !== void 0 && !isStringArray(value.tags)) {
    return false;
  }
  if ("contexts" in value && value.contexts !== void 0 && !isStringArray(value.contexts)) {
    return false;
  }
  if ("skills" in value && value.skills !== void 0 && !isStringArray(value.skills)) {
    return false;
  }
  if ("project" in value && value.project !== void 0 && typeof value.project !== "string") {
    return false;
  }
  if ("phase" in value && value.phase !== void 0 && typeof value.phase !== "string") {
    return false;
  }
  if ("projectSlug" in value) {
    ...
    if (value.projectSlug !== void 0 && typeof value.projectSlug !== "string") {
      return false;
    }
  }
  return true;
...
var isWebviewToHostMessage = (value) => {
  ...
  if (value.type === "RequestTaskSnapshot" || value.type === "ShowKanbanBoard" || value.type === "CloseTaskEditor") {
    return true;
  }
  ...
  if (value.type === "UpdateTask") {
    return isObject(value.payload) && typeof value.payload.taskId === "string" && isTaskInput(value.payload.updates, false);
  }
  if (value.type === "DeleteTask") {
    return isObject(value.payload) && typeof value.payload.taskId === "string";
  }
  if (value.type === "MoveTask") {
    if (!isObject(value.payload) || typeof value.payload.taskId !== "string" || !isTaskStage(value.payload.targetStage)) {
      return false;
    }
    if ("order" in value.payload && value.payload.order !== void 0 && (typeof value.payload.order !== "number" || !Number.isFinite(value.payload.order))) {
      return false;
    }
    return true;
  }
  if (value.type === "ReorderTask") {
    return isObject(value.payload) && typeof value.payload.taskId === "string" && typeof value.payload.newOrder === "number" && Number.isFinite(value.payload.newOrder);
  }
  if (value.type === "OpenTaskEditor") {
    if (value.payload === void 0) {
      return true;
    }
    return isObject(value.payload) && (!("taskId" in value.payload) || value.payload.taskId === void 0 || typeof value.payload.taskId === "string") && (!("focusTargets" in value.payload) || value.payload.focusTargets === void 0 || Array.isArray(value.payload.focusTargets) && value.payload.focusTargets.every(isValidationFocusTarget));
  }
  if (value.type === "SaveTask") {
    return isObject(value.payload) && isTaskInput(value.payload.task, true) && (!("taskId" in value.payload) || value.payload.taskId === void 0 || typeof value.payload.taskId === "string");
  }
  if (value.type === "OpenSettings") {
    if (value.payload === void 0) {
      return true;
    }
    return isObject(value.payload) && (!("projectSlug" in value.payload) || value.payload.projectSlug === void 0 || typeof value.payload.projectSlug === "string");
  }
  ...
  if (value.type === "ResetSection") {
    return isObject(value.payload) && isSettingsSection(value.payload.section) && (!("projectSlug" in value.payload) || value.payload.projectSlug === void 0 || typeof value.payload.projectSlug === "string");
  }
  ...
  if (value.type === "RunStage" || value.type === "RunAllStages" || value.type === "QueueStage" || value.type === "QueueAllStages" || value.type === "CancelRun" || value.type === "RetryRun") {
    return isObject(value.payload) && typeof value.payload.taskId === "string";
  }
  return false;
};
...
var getMockSidebarResponse = (provider, selectedTask) => {
...
var resolveSidebarChatResponse = async (provider, message, selectedTask, alibabaService) => {
  if (provider !== "alibaba") {
...
var SidebarProvider = class {
  constructor(extensionUri, taskScanner, queueService, alibabaService) {
...
<html lang="en">
  ...
  async postOrchestratorResponse(provider, message, selectedTask) {
    if (!this.webviewView) {
      return;
    }
  ...
  postRunnerStateChanged(taskId, state, timestamp) {
    if (!this.webviewView) {
      return;
    }
    const message = {
...
var KanbanPanel = class _KanbanPanel {
  constructor(panel, extensionUri, taskScanner, taskService, settingsService, queueService) {
    this._disposables = [];
    this._panel = panel;
  ...
  static createOrShow(extensionUri, taskScanner, taskService, settingsService, queueService) {
    ...
    _KanbanPanel.currentPanel = new _KanbanPanel(
      panel,
      extensionUri,
      taskScanner,
      taskService,
  ...
  async handleWebviewMessage(rawMessage) {
    ...
    if (rawMessage.type === "CancelRun") {
      ...
      if (!cancelled) {
        void vscode2.window.showWarningMessage(`No queued/running task found: ${rawMessage.payload.taskId}`);
      }
      return;
    }
    if (rawMessage.type === "RetryRun") {
      ...
      if (!resolvedTask) {
        void vscode2.window.showWarningMessage(`Task not found: ${rawMessage.payload.taskId}`);
        return;
      }
      const result = await this.queueService.retry(resolvedTask.id);
      if (!result.ok) {
        void vscode2.window.showWarningMessage("Retry is only available for failed runs.");
      }
      return;
    }
    ...
    if (rawMessage.type === "CreateTask") {
      ...
      if (!title) {
        void vscode2.window.showWarningMessage("Task title is required.");
        return;
      }
    ...
    if (rawMessage.type === "DeleteTask") {
      ...
      try {
        ...
        void vscode2.window.showErrorMessage(`Failed to delete task: ${message}`);
      }
      return;
    }
    if (rawMessage.type === "OpenTaskEditor") {
      ...
      if (!taskIdentifier) {
        void vscode2.window.showWarningMessage("Task identifier is required to open the task editor.");
        return;
      }
      const resolvedTask = await this.resolveTask(taskIdentifier);
      if (!resolvedTask) {
        void vscode2.window.showWarningMessage(`Task not found: ${taskIdentifier}`);
        return;
      }
      try {
        await vscode2.commands.executeCommand("kanban2code.openTaskEditor", resolvedTask.id);
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        void vscode2.window.showErrorMessage(`Failed to open task editor: ${message}`);
      }
      return;
    }
    if (rawMessage.type === "OpenSettings") {
      await vscode2.commands.executeCommand("kanban2code.openSettings", rawMessage.payload);
      return;
    }
  ...
  async handleQueueIntent(taskIdentifier, scope) {
    ...
    if (!result.ok && result.reason === "duplicate") {
      void vscode2.window.showWarningMessage("Task is already queued or running.");
      return;
    }
    if (!result.ok && result.validation) {
      const message = result.validation.errors[0]?.message ?? "Task validation failed before run.";
      void vscode2.window.showWarningMessage(message);
    }
  }
...
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta http-equiv="Content-Security-Policy" content="default-src 'none'; font-src https://fonts.gstatic.com; style-src ${webview.cspSource} 'unsafe-inline' https://fonts.googleapis.com; script-src 'nonce-${nonce}' 'unsafe-inline'; img-src data: https:;" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="preconnect" href="https://fonts.googleapis.com" />
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600;700&family=Noto+Sans+Mono:wght@400;500;600;700&display=swap" rel="stylesheet" />
...
// src/webview/TaskEditorPanel.ts
var vscode3 = __toESM(require("vscode"));
var TaskEditorPanel = class _TaskEditorPanel {
  constructor(panel, extensionUri, taskService, initialTaskPath, initialFocusTargets) {
    this.disposables = [];
    this.currentTaskPath = null;
  ...
  static {
    this.viewType = "kanban2code-task-editor";
  }
  static {
    this.saveEmitter = new vscode3.EventEmitter();
  }
  static {
    this.onDidSaveTask = _TaskEditorPanel.saveEmitter.event;
  }
  static createOrShow(extensionUri, taskService, taskPath, focusTargets) {
    const column = vscode3.window.activeTextEditor ? vscode3.window.activeTextEditor.viewColumn : void 0;
    if (_TaskEditorPanel.currentPanel) {
      _TaskEditorPanel.currentPanel.panel.reveal(column);
      if (taskPath && taskPath !== _TaskEditorPanel.currentPanel.currentTaskPath) {
        void _TaskEditorPanel.currentPanel.loadTask(taskPath, focusTargets);
      } else if (focusTargets?.length) {
        _TaskEditorPanel.currentPanel.focusFields(focusTargets);
      }
      return;
    }
    const panel = vscode3.window.createWebviewPanel(
      _TaskEditorPanel.viewType,
      "Task Editor",
      column || vscode3.ViewColumn.One,
      {
        enableScripts: true,
        localResourceRoots: [vscode3.Uri.joinPath(extensionUri, "dist")],
        retainContextWhenHidden: true
      }
    );
    _TaskEditorPanel.currentPanel = new _TaskEditorPanel(
      panel,
      extensionUri,
      taskService,
      taskPath,
  ...
  dispose() {
    ...
    _TaskEditorPanel.currentPanel = void 0;
    this.panel.dispose();
  ...
  async handleSaveTask(message) {
    if (!this.currentTaskPath) {
      void vscode3.window.showWarningMessage("No task is currently loaded.");
      return;
    }
    try {
      await this.taskService.updateTask(this.currentTaskPath, message.payload.task);
      _TaskEditorPanel.saveEmitter.fire({ taskPath: this.currentTaskPath });
      await this.loadTask(this.currentTaskPath);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      void vscode3.window.showErrorMessage(`Failed to save task: ${errorMessage}`);
    }
  }
  async loadTask(taskPath, focusTargets) {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      void vscode3.window.showErrorMessage(`Failed to load task editor: ${errorMessage}`);
      this.dispose();
    }
  }
  focusFields(focusTargets) {
  ...
  getHtmlForWebview(webview) {
    const scriptUri = webview.asWebviewUri(
      vscode3.Uri.joinPath(this.extensionUri, "dist", "taskeditor.js")
    );
    const styleUri = webview.asWebviewUri(
      vscode3.Uri.joinPath(this.extensionUri, "dist", "taskeditor.css")
    );
...
var SettingsPanel = class _SettingsPanel {
  constructor(panel, extensionUri, settingsService, projectSlug) {
    this.disposables = [];
    this.isDisposed = false;
    this.panel = panel;
    this.extensionUri = extensionUri;
  ...
  static {
    this.viewType = "kanban2code-settings";
  }
  static createOrShow(extensionUri, settingsService, projectSlug) {
    ...
    if (_SettingsPanel.currentPanel) {
      ...
      void _SettingsPanel.currentPanel.openSettings(projectSlug);
      return;
    }
    const panel = vscode4.window.createWebviewPanel(
      ...
      {
        enableScripts: true,
        localResourceRoots: [vscode4.Uri.joinPath(extensionUri, "dist")],
        retainContextWhenHidden: true
      }
    );
    _SettingsPanel.currentPanel = new _SettingsPanel(panel, extensionUri, settingsService, projectSlug);
  }
  dispose() {
    if (this.isDisposed) {
      return;
  ...
  async handleOpenSettings(message) {
  ...
  async handleSaveSettings(message) {
    try {
      ...
      await this.settingsService.updateSettings(message.payload.settings, projectSlug);
      await this.postSettings(projectSlug);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      void vscode4.window.showErrorMessage(`Failed to save settings: ${errorMessage}`);
    }
  }
  async handleResetSection(message) {
    try {
      const projectSlug = this.resolveProjectSlug(message.payload.projectSlug);
      await this.settingsService.resetSection(message.payload.section, projectSlug);
      await this.postSettings(projectSlug);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      void vscode4.window.showErrorMessage(`Failed to reset settings section: ${errorMessage}`);
    }
  }
  async handleResetToDefaults(message) {
    try {
      const projectSlug = this.resolveProjectSlug(message.payload?.projectSlug);
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      void vscode4.window.showErrorMessage(`Failed to reset settings defaults: ${errorMessage}`);
    }
  }
  async openSettings(projectSlug) {
    this.currentProjectSlug = projectSlug;
    await this.postSettings(projectSlug);
  }
  ...
  async postSettings(projectSlug = this.currentProjectSlug) {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      void vscode4.window.showErrorMessage(`Failed to load settings: ${errorMessage}`);
    }
  }
  getHtmlForWebview(webview) {
    const scriptUri = webview.asWebviewUri(
      vscode4.Uri.joinPath(this.extensionUri, "dist", "settings.js")
    );
    const styleUri = webview.asWebviewUri(
      vscode4.Uri.joinPath(this.extensionUri, "dist", "settings.css")
    );
...
var asTaskStage = (value) => {
  if (typeof value !== "string") {
    return "unknown";
  }
  const normalized = value.trim().toLowerCase();
  return STAGES.has(normalized) ? normalized : "unknown";
};
...
var parseTaskMarkdown = (input) => {
  try {
    const parsed = (0, import_gray_matter.default)(input);
    return {
...
var serializeTaskMarkdown = (task) => {
  const normalized = normalizeFrontmatter(task.frontmatter);
  const frontmatterForWrite = {
    ...normalized,
  ...
  const serializedFrontmatter = Object.fromEntries(
    Object.entries(frontmatterForWrite).filter(([, value]) => value !== void 0)
  );
  return import_gray_matter.default.stringify(task.body, serializedFrontmatter);
};

// src/services/task-scanner.ts
var INBOX_GLOB = ".kanban2code/inbox/**/*.md";
var PROJECT_GLOB = ".kanban2code/projects/**/*.md";
var WATCH_GLOB = ".kanban2code/**/*.md";
var normalizeToPosix = (value) => value.split(path.sep).join(path.posix.sep);
...
var normalizeOrder = (value) => {
  if (typeof value !== "number" || !Number.isFinite(value)) {
    return void 0;
  }
  return value;
};
var toSearchable = (value) => value.trim().toLowerCase();
var getTitleFromBody = (body) => {
  const headingMatch = body.match(/^#\s+(.+)$/m);
  if (!headingMatch) {
    return void 0;
  }
  const heading = headingMatch[1]?.trim();
  return heading && heading.length > 0 ? heading : void 0;
};
var getDescriptionFromBody = (body) => {
  const paragraphs = body.split(/\n\s*\n/g).map((paragraph) => paragraph.replace(/\s+/g, " ").trim()).filter((paragraph) => paragraph.length > 0);
...
var extractProjectFromPath = (relativePath) => {
  const match = relativePath.match(/^\.kanban2code\/projects\/([^/]+)\//);
  return match?.[1];
};
var sortByCreatedAt = (tasks) => {
...
var TaskScanner = class {
  constructor(runtime) {
    ...
    this.refreshEmitter = this.runtime.createEventEmitter();
    this.disposables = [];
    this.cachedTasks = null;
    this.onDidRefresh = this.refreshEmitter.event;
  ...
  async scan(options2 = {}) {
    ...
    const filtered = this.applyFilters(this.cachedTasks, options2.filters);
  ...
  async readTask(uri) {
    try {
      ...
      const title = typeof parsed.frontmatter.title === "string" && parsed.frontmatter.title.trim().length > 0 ? parsed.frontmatter.title.trim() : fallbackTitle;
      const project = typeof parsed.frontmatter.project === "string" && parsed.frontmatter.project.trim().length > 0 ? parsed.frontmatter.project.trim() : extractProjectFromPath(relativePath);
      return {
        id,
        taskId,
        title,
        ...
        role: typeof parsed.frontmatter.role === "string" ? parsed.frontmatter.role : typeof parsed.frontmatter.agent === "string" ? parsed.frontmatter.agent : void 0,
        project,
...
var FILE_NAME_MAX_SLUG_LENGTH = 50;
var DEFAULT_STAGE = "inbox";
var DEFAULT_BODY = "";
var hasValue = (value) => typeof value === "string" && value.trim().length > 0;
var normalizeStringArray = (value) => Array.isArray(value) ? [...value] : [];
var toKebabCase = (value) => {
  const normalized = value.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9\s-]/g, " ").trim().replace(/[\s_-]+/g, "-").replace(/^-+|-+$/g, "");
  if (!normalized) {
    return "untitled";
  }
  return normalized.slice(0, FILE_NAME_MAX_SLUG_LENGTH).replace(/-+$/g, "") || "untitled";
};
var cloneFrontmatter = (frontmatter) => ({
  ...frontmatter,
...
var TaskService = class {
  constructor(workspaceRoot, options2 = {}) {
  ...
  async createTask(data) {
    ...
    const title = hasValue(data.title) ? data.title.trim() : void 0;
  ...
  async updateTask(filePath, changes) {
    const existing = await this.readTask(filePath);
    const nextFrontmatter = cloneFrontmatter(existing.frontmatter);
    if (changes.stage !== void 0) nextFrontmatter.stage = changes.stage;
    if (changes.order !== void 0) nextFrontmatter.order = changes.order;
    if (changes.title !== void 0) nextFrontmatter.title = changes.title;
    if (changes.role !== void 0) nextFrontmatter.role = changes.role;
    if (changes.agent !== void 0) nextFrontmatter.agent = changes.agent;
    if (changes.provider !== void 0) nextFrontmatter.provider = changes.provider;
    if (changes.model !== void 0) nextFrontmatter.model = changes.model;
    if (changes.profile !== void 0) nextFrontmatter.profile = changes.profile;
    if (changes.priority !== void 0) nextFrontmatter.priority = changes.priority;
    if (changes.tags !== void 0) nextFrontmatter.tags = normalizeStringArray(changes.tags);
    if (changes.contexts !== void 0) nextFrontmatter.contexts = normalizeStringArray(changes.contexts);
    if (changes.skills !== void 0) nextFrontmatter.skills = normalizeStringArray(changes.skills);
    if (changes.project !== void 0) nextFrontmatter.project = changes.project;
    if (changes.phase !== void 0) nextFrontmatter.phase = changes.phase;
    const updated = {
      frontmatter: nextFrontmatter,
      body: changes.body !== void 0 ? changes.body : existing.body
    };
    const deps = await this.getRuntimeDependencies();
    await this.writeTask(filePath, updated, deps);
    return updated;
  }
  async deleteTask(filePath) {
    const deps = await this.getRuntimeDependencies();
    const uri = deps.toFileUri(this.toAbsolutePath(filePath));
    try {
  ...
  async moveTask(filePath, newStage, projectSlug) {
    const deps = await this.getRuntimeDependencies();
    ...
    const currentDirectory = path2.dirname(filePath);
  ...
  resolveDirectory(projectSlug) {
    if (hasValue(projectSlug)) {
      return path2.join(".kanban2code", "projects", projectSlug.trim());
    }
    return path2.join(".kanban2code", "inbox");
  }
  async writeTask(filePath, task, deps) {
    const parentDirectory = path2.dirname(filePath);
  ...
  async buildUniqueTaskPath(directory, baseName, deps) {
    let counter = 0;
    while (true) {
      const suffix = counter === 0 ? "" : `-${counter}`;
      const candidate = path2.join(directory, `${baseName}${suffix}.md`);
  ...
  async pathExists(filePath, deps) {
    const fileUri = deps.toFileUri(this.toAbsolutePath(filePath));
  ...
  toAbsolutePath(relativePath) {
    return path2.join(this.workspaceRoot, relativePath);
  }
  getNow() {
  ...
  async getRuntimeDependencies() {
    ...
    const vscode6 = await import("vscode");
    this.runtimeDependencies = {
      fs: vscode6.workspace.fs,
      toFileUri: vscode6.Uri.file
    };
    return this.runtimeDependencies;
  }
};
...
var createDefaultDeps = () => {
  ...
  return {
    fs: vscode6.workspace.fs,
    toFileUri: (filePath) => vscode6.Uri.file(filePath)
  };
};
...
var SettingsService = class {
  constructor(workspaceRoot, deps = createDefaultDeps()) {
    this.workspaceRoot = workspaceRoot;
    this.deps = deps;
  }
  async getSettings(projectSlug) {
  ...
  async getEffectiveMapping(stage, projectSlug) {
  ...
  async updateSettings(settings, projectSlug) {
    ...
    const updatedScopeSettings = this.deepMerge(currentScopeSettings, settings);
  ...
  async resetSection(section, projectSlug) {
    const scopeFilePath = projectSlug ? this.getProjectPath(projectSlug) : this.getGlobalPath();
    const currentScopeSettings = await this.readSettingsFile(scopeFilePath);
    const updatedScopeSettings = {
      ...currentScopeSettings,
      [section]: clone(DEFAULT_SETTINGS[section])
    };
    await this.writeSettingsFile(scopeFilePath, updatedScopeSettings);
  }
  async resetToDefaults(projectSlug) {
    if (projectSlug) {
  ...
  validateProviderModel(provider, model, settings) {
  ...
  validateSettings(settings) {
    ...
    for (const [stage, mapping] of Object.entries(settings.stageRuntimeMapping)) {
      ...
      if (profileConfig && (profileConfig.provider !== mapping.provider || profileConfig.model !== mapping.model)) {
        errors.push(
  ...
  validateMapping(provider, model) {
    return this.validateProviderModel(provider, model);
  }
  getFallbackMapping(stage) {
  ...
  async readSettingsFile(relativePath) {
    const uri = this.deps.toFileUri(path3.join(this.workspaceRoot, relativePath));
    try {
  ...
  mergeSettings(fallback, global, project) {
    return this.deepMerge(this.deepMerge(fallback, global), project);
  }
  async writeSettingsFile(relativePath, settings) {
    const absolutePath = path3.join(this.workspaceRoot, relativePath);
    await this.deps.fs.createDirectory(this.deps.toFileUri(path3.dirname(absolutePath)));
    await this.deps.fs.writeFile(
      this.deps.toFileUri(absolutePath),
      Buffer.from(JSON.stringify(settings, null, 2), "utf8")
    );
  }
  async getValidatedEffectiveSettings(scopedSettings, projectSlug) {
    if (!projectSlug) {
...
var hasValue2 = (value) => typeof value === "string" && value.trim().length > 0;
var normalizeStage = (value) => value.trim().toLowerCase();
var inferProjectSlugFromPath = (taskPath) => {
  ...
  return hasValue2(projectSlug) ? projectSlug.trim() : void 0;
};
var isTerminalState = (state) => state === "success" || state === "failed" || state === "cancelled";
var isAllowedStateTransition = (from, to) => {
  if (!from) {
    return true;
  }
  if (from === to) {
...
var QueueService = class {
  constructor(workspaceRoot, taskService, settingsService, options2 = {}) {
    this.workspaceRoot = workspaceRoot;
    this.taskService = taskService;
    this.settingsService = settingsService;
    this.options = options2;
    this.queue = [];
    this.stateByTaskId = /* @__PURE__ */ new Map();
    this.pendingRunIntents = /* @__PURE__ */ new Map();
    this.runningTaskIds = /* @__PURE__ */ new Set();
    this.cancellationRequested = /* @__PURE__ */ new Set();
    this.emitter = new TypedQueueEmitter();
  }
  async enqueue(taskId, scope, projectSlug) {
  ...
  async retry(taskId, scope = "stage") {
  ...
  onDidStateChange(listener) {
    this.emitter.on("state", listener);
    return () => {
      this.emitter.off("state", listener);
    };
  }
  onDidQueueChange(listener) {
  ...
  onDidRunnerStateChange(listener) {
  ...
  onDidQueueSnapshotChange(listener) {
    const wrapped = () => listener();
    this.emitter.on("queue", wrapped);
    return {
      dispose: () => {
  ...
  async applyExecutionDefaults(taskPath, task, projectSlug) {
    const mapping = await this.settingsService.getEffectiveMapping(task.frontmatter.stage, projectSlug);
    const updates = {};
    if (!hasValue2(task.frontmatter.role)) {
      updates.role = mapping.role;
    }
    if (!hasValue2(task.frontmatter.provider)) {
      updates.provider = mapping.provider;
    }
    if (!hasValue2(task.frontmatter.model)) {
      updates.model = mapping.model;
    }
    if (!hasValue2(task.frontmatter.profile)) {
  ...
  async validateTaskForRun(taskPath, frontmatter, settings, scope, projectSlug) {
    const errors = [];
    const missingRequiredFields = [];
    const focusTargets = [];
    const addIssue = (error, options2) => {
      errors.push(error);
      if (options2?.missingField && !missingRequiredFields.includes(options2.missingField)) {
        missingRequiredFields.push(options2.missingField);
      }
      if (options2?.focusField) {
        const exists = focusTargets.some(
          (entry) => entry.field === options2.focusField && entry.stage === options2.stage
        );
        if (!exists) {
          focusTargets.push({
            field: options2.focusField,
            stage: options2.stage
          });
        }
      }
    };
    if (!hasValue2(frontmatter.title)) {
      addIssue(
    ...
    const hasValidStage = hasValue2(stage) && normalizedStage !== "unknown";
    if (!hasValidStage) {
      addIssue(
    ...
    if (scope === "stage") {
      this.validateExecutionMapping(
        ...
        {
          scope: "stage",
          stage: hasValidStage ? stage : void 0,
          focusField: "pipeline"
        }
      );
    } else {
      const pipelineStages = this.getPipelineStages(settings);
      const stageMappingByName = /* @__PURE__ */ new Map();
      ...
      for (const [stageName, mapping] of stageMappingByName.entries()) {
        ...
        this.validateExecutionMapping(mapping, settings, addIssue, {
          scope: "pipeline",
          stage: stageName,
          focusField: isCurrentStage ? void 0 : "pipeline"
        });
      }
    }
  ...
  validateLocation(taskPath, frontmatter, addIssue) {
  ...
  validateExecutionMapping(mapping, settings, addIssue, context) {
    const prefix = context.scope === "pipeline" && context.stage ? `Pipeline step '${context.stage}'` : "Task execution mapping";
  ...
  async findUnavailableSkills(skills) {
    const unavailable = [];
    for (const raw of skills) {
      ...
      const skillPath = path4.join(this.workspaceRoot, ".kanban2code", "_context", "skills", normalized);
  ...
  resolveContextCandidates(name) {
    const normalized = name.endsWith(".md") ? name : `${name}.md`;
    return [
      path4.join(this.workspaceRoot, ".kanban2code", "_context", normalized),
      path4.join(this.workspaceRoot, ".kanban2code", "_context", "skills", normalized)
    ];
  }
...
var DEFAULT_MODEL = "glm-5";
var createDefaultDeps2 = () => ({
...
var normalizeEndpoint = (endpoint) => {
  const trimmed = endpoint.trim().replace(/\/+$/u, "");
  return trimmed.endsWith("/chat/completions") ? trimmed : `${trimmed}/chat/completions`;
};
var getTaskScopeLabel = (selectedTask) => selectedTask ? `${selectedTask.stage} task "${selectedTask.title}"` : "general chat";
var buildUserPrompt = (request) => {
...
Task summary: ${request.selectedTask.description}` : "";
  return [
    ...
    request.selectedTask.project ? `Project: ${request.selectedTask.project}` : void 0,
    description.trim() || void 0,
    "",
    `User message: ${request.message}`
  ].filter((part) => typeof part === "string" && part.length > 0).join("\n");
};
var getProviderConfig = (settings) => settings.providersAndModels.providers.alibaba;
var getConfiguredApiKey = (env) => {
...
var getErrorMessage = (payload) => {
  ...
  if (!isRecord3(error) || typeof error.message !== "string" || error.message.trim().length === 0) {
    return void 0;
  }
...
var getAssistantMessage = (payload) => {
  ...
  if (Array.isArray(content)) {
    const joined = content.map((part) => part && typeof part.text === "string" ? part.text : "").join("").trim();
    return joined.length > 0 ? joined : void 0;
  }
  return void 0;
};
var AlibabaService = class {
  constructor(workspaceRoot, settingsService, deps = createDefaultDeps2(), options2 = {}) {
    this.workspaceRoot = workspaceRoot;
    this.settingsService = settingsService;
    this.deps = deps;
    this.envSearchRoots = this.buildEnvSearchRoots(options2.envSearchRoots ?? []);
  }
  async sendMessage(request, projectSlug) {
    const settings = await this.settingsService.getSettings(projectSlug);
    ...
    const response = await this.deps.fetch(normalizeEndpoint(endpoint), {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You are the Kanban2Code sidebar orchestrator in an interactive VS Code session. Give concise, practical help and keep responses grounded in the provided task scope."
          },
          {
            role: "system",
    ...
    if (!response.ok) {
      ...
      const payload2 = rawText ? parseJson(rawText) : await this.safeReadJson(response);
      const message2 = getErrorMessage(payload2) ?? rawText ?? `Request failed with status ${response.status}.`;
      throw new Error(`Alibaba request failed: ${message2}`);
    }
  ...
  async getApiKey() {
    ...
    for (const root of this.envSearchRoots) {
      const envFilePath = path5.join(root, ".env");
      try {
  ...
  buildEnvSearchRoots(extraRoots) {
    ...
    for (const candidate of candidates) {
      ...
      const normalized = path5.resolve(trimmed);
...
function activate(context) {
  const scannerRuntime = {
    findFiles: (globPattern) => vscode5.workspace.findFiles(globPattern),
    readFile: (uri) => vscode5.workspace.fs.readFile(uri),
    toRelativePath: (uri) => vscode5.workspace.asRelativePath(uri, false),
    createWatcher: (globPattern) => vscode5.workspace.createFileSystemWatcher(globPattern),
    createEventEmitter: () => new vscode5.EventEmitter()
  };
  ...
  const alibabaService = new AlibabaService(workspaceRoot ?? "", settingsService, void 0, {
  ...
  const queueService = new QueueService(workspaceRoot ?? "", taskService, settingsService, {
    promptForValidation: async (taskPath, validation) => {
      ...
      if (firstError) {
        void vscode5.window.showWarningMessage(firstError.message);
      }
      await vscode5.commands.executeCommand("kanban2code.openTaskEditor", {
        taskId: taskPath,
  ...
  context.subscriptions.push(
    vscode5.window.registerWebviewViewProvider(SidebarProvider.viewType, sidebarProvider)
  );
  context.subscriptions.push(
  ...
  context.subscriptions.push(
    vscode5.commands.registerCommand("kanban2code.openBoard", () => {
      if (!workspaceRoot) {
  ...
  context.subscriptions.push(
    vscode5.commands.registerCommand("kanban2code.openTaskEditor", (arg) => {
      if (!workspaceRoot) {
        void vscode5.window.showWarningMessage("Open a workspace folder to use the task editor.");
        return;
      }
      const taskPath = typeof arg === "string" ? arg : arg && typeof arg === "object" && "taskId" in arg && typeof arg.taskId === "string" ? arg.taskId : void 0;
      const focusTargets = arg && typeof arg === "object" && "focusTargets" in arg && Array.isArray(arg.focusTargets) ? arg.focusTargets ?? [] : void 0;
      TaskEditorPanel.createOrShow(context.extensionUri, taskService, taskPath, focusTargets);
    })
  );
  context.subscriptions.push(
    vscode5.commands.registerCommand("kanban2code.openSettings", (arg) => {
      if (!workspaceRoot) {
        void vscode5.window.showWarningMessage("Open a workspace folder to configure Kanban2Code settings.");
        return;
      }
      const projectSlug = arg && typeof arg === "object" && "projectSlug" in arg && typeof arg.projectSlug === "string" ? arg.projectSlug : void 0;
